...
export default function Sidebar({ translations, activeSection, onSectionChange, language, onLanguageChange, isCollapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <>
      {/* Modern Tech Toggle Button */}
      <button
        onClick={() => onToggleCollapse(!isCollapsed)}
        className="fixed top-6 left-6 z-50 w-12 h-12 bg-gradient-to-br from-[#00ff88] to-[#00d4ff] rounded-xl flex items-center justify-center shadow-2xl hover:scale-105 transition-all duration-300 group backdrop-blur-sm border border-white/10"
      >
        <div className="absolute inset-0 bg-gradient-to-br from-[#00ff88] to-[#00d4ff] rounded-xl blur opacity-20 group-hover:opacity-40 transition-opacity duration-300"></div>
      ...
      {/* Modern Tech Sidebar */}
      <aside className={`fixed left-0 top-0 h-full bg-gradient-to-br from-gray-900/95 to-gray-800/95 backdrop-blur-xl border-r border-white/10 shadow-2xl z-40 overflow-y-auto transition-all duration-500 ${
