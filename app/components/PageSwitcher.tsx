...
export default function PageSwitcher({ translations, language, isSidebarCollapsed = true }: PageSwitcherProps) {
  ...
  return (
    <>
      {/* Background Header - Adjusts with sidebar */}
      <div className={`fixed top-0 left-0 right-0 z-40 bg-black/10 backdrop-blur-2xl border-b border-white/5 transition-all duration-500 ${
        isSidebarCollapsed ? 'lg:ml-0' : 'lg:ml-64'
      }`}>
        <div className="w-full h-16"></div>
      </div>
      
      {/* Fixed Center Buttons - Always centered */}
