...
export default function PageSwitcher({ translations, language }: PageSwitcherProps) {
  ...
  return (
    <div className="fixed top-6 sm:top-8 left-1/2 transform -translate-x-1/2 z-50">
      <div className="bg-gradient-to-r from-gray-900/90 to-gray-800/90 backdrop-blur-xl border border-white/20 rounded-2xl p-1.5 sm:p-2 shadow-2xl">
        <div className="flex gap-1 sm:gap-2">
          {/* Professional Page Button */}
          <button
            onClick={() => handlePageSwitch('professional')}
            className={`flex items-center space-x-2 sm:space-x-3 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl transition-all duration-300 ${
              isProfessionalPage
